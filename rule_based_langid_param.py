...
# несколько наборов весов, чтобы погонять разные варианты
WEIGHT_PRESETS = {
    "base": {
        "stop": 3.0,
        "special": 4.0,
...
def score_language(text: str, lang_code: str) -> float:
    ...
    for gram in rules.get("bigrams", set()):
        score += WEIGHTS["bigram"] * bigram_counts.get(gram, 0)

    # 3-grams
    for gram in rules.get("trigrams", set()):
        score += WEIGHTS["trigram"] * trigram_counts.get(gram, 0)

    # 4-grams
    for gram in rules.get("fourgrams", set()):
        score += WEIGHTS["fourgram"] * fourgram_counts.get(gram, 0)
...
def run_experiment(
    ...
    """
    Один прогон:
      - загружаем корпус с ограничением max_sent_per_lang
      - делим train/test
      - обучаем n-граммы с top_k
      - считаем accuracy и confusion
    """

    global WEIGHTS
    WEIGHTS = WEIGHT_PRESETS[weight_name]

    texts, labels = load_corpus_from_dir(
        data_dir, max_sent_per_lang=max_sent_per_lang
    )
    # на всякий, если у какого-то языка меньше предложений – просто будет меньше
    total = len(texts)
...
if __name__ == "__main__":
    ...
    SENT_SIZES = [50000, 80000, 100000]        # сколько предложений на язык
    TOP_K_VALUES = [20, 30, 40]                # топ-N n-грамм
    WEIGHT_NAMES = ["base", "strong_ngrams", "strong_special"]

    results = []      # сюда сложим (size, top_k, weights, acc)
    best_conf = None  # confusion для лучшей конфигурации
    best_setup = None

    for max_sent in SENT_SIZES:
        for top_k in TOP_K_VALUES:
            for w_name in WEIGHT_NAMES:
                print(
                    f"\nRunning experiment: "
                    f"max_sent={max_sent}, top_k={top_k}, weights={w_name}"
                )
                acc, conf, total = run_experiment(
                    DATA_DIR,
                    max_sent_per_lang=max_sent,
                    top_k=top_k,
                    weight_name=w_name,
                )
                results.append((max_sent, top_k, w_name, acc))

                if best_setup is None or acc > best_setup[3]:
                    best_setup = (max_sent, top_k, w_name, acc)
                    best_conf = conf

    # Табличка с точностями
    print("\n=== Accuracy table ===")
    print("max_sent\ttop_k\tweights\taccuracy")
    for max_sent, top_k, w_name, acc in results:
        print(f"{max_sent}\t{top_k}\t{w_name}\t{acc:.4f}")

    # Конфьюжены только по ОШИБКАМ для лучшей конфигурации
    print("\n=== Best configuration ===")
    ms, tk, wname, acc = best_setup
    print(f"max_sent={ms}, top_k={tk}, weights={wname}, accuracy={acc:.4f}")

    print("\nTop confusions (gold -> pred, count), only errors:")
    mis = [((g, p), c) for (g, p), c in best_conf.items() if g != p]
    mis_sorted = sorted(mis, key=lambda x: -x[1])[:20]
    for (gold, pred), cnt in mis_sorted:
